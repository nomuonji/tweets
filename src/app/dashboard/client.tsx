...
export function DashboardClient({
  ...
  const handleStatusChange = async (draftId: string, newStatus: DraftDoc['status']) => {
    try {
      const response = await fetch(`/api/drafts/${draftId}`, {
        method: 'PUT',
                        ...
                        <li key={draft.id} className="rounded-lg border border-border bg-background p-4">
                          <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                            <div className="flex flex-col">
                              ...
                              {draft.generatedBy && (
                                <span className="mt-1 text-xs text-muted-foreground">Generated by {toTitleCase(draft.generatedBy)}</span>
                              )}
                            </div>
                            <select
                              value={draft.status}
